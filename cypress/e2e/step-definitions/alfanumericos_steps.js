...
Given("el usuario configura el nombre como {string}", (nombre) => {
  jsonSolicitud.nombres = nombre;
});

When("el usuario envía la solicitud de recogida con un nombre no válido", () => {
  cy.request({
    method: "POST",
...
Then("el servicio responde con el código {int} indicando error en el nombre", (statusCode) => {
...
Then("el mensaje es {string}", (mensaje) => {
  cy.get("@solicitudAlfanumerica").then((response) => {
    expect(response.body.message).to.eq(mensaje);
  });
});
